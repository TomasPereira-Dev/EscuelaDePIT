            ...
            <section className="flex flex-col items-center gap-8 lg:flex-row-reverse lg:items-center lg:justify-between lg:gap-8">
                <figure>
                    <picture>
                        ...
                        <img className="w-full" src="./liftogt.png" alt="el doctor liftogt dando catedra junto a un paciente" />
                    </picture>
                    <figcaption className="text-sm text-text1 italic">El doctor John Liftogt enseñando junto a un paciente</figcaption>
                </figure>
                <div className="flex flex-col gap-4">
                    <h2 className="text-3xl text-text1 text-center font-bold lg:text-5xl md:text-left">Nuestra Historia</h2>
                    <p className="text-text1 md:max-w-[64ch]">En 2010, el Dr. John Lyftogt descubrió en Nueva Zelanda un tratamiento efectivo para el dolor causado por las fibras C. Fue el primero en usar D5W (dextrosa al 5% en agua estéril) para tratar los nervios dolorosos e inhibir el dolor neuropático crónico En 2014.</p>
                    <p className="text-text1 md:max-w-[64ch]">tras el Congreso Argentino de Medicina General y Familiar, los doctores Zárate Miguel, Ninin Laura y Fuso Miguel decidieron invitar al Dr. John Lyftogt a dictar un curso en Argentina.</p>
                    <p className="text-text1 md:max-w-[64ch]">La llegada del Dr. Lyftogt y su curso básico permitió que todos los asistentes pudieran, por primera vez, ayudar a sus pacientes a disminuir su dolor crónico y mejorar su calidad de vida. <span className="underline cursor-pointer lg:hidden" onClick={() => {setReadMore(!readMore)}}> {!readMore && 'Leer más'} </span></p>
                    <p className={`${readMore ? 'block' : 'hidden'}  text-text1 lg:block md:max-w-[64ch]`}>En 2024, John Lyftogt anunció su retiro de la enseñanza a nivel mundial y mencionó que había cuatro médicos en Argentina capacitados para enseñar su técnica. Conmovidos, decidimos continuar su legado y difundir la técnica. Así nació la “Escuela de PIT”, un proyecto altruista que honra a John y Maria y enseña la práctica de PIT a la mayor cantidad de personas posible. <span className="underline cursor-pointer lg:hidden" onClick={() => {setReadMore(!readMore)}}>{readMore && 'Leer menos'}</span></p>
                </div>
            </section>
